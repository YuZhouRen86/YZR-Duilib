...
bool PhysicalToLogicalPointWrapper(HWND hWnd, LPPOINT lpPoint)
{
	typedef BOOL(WINAPI *PhysicalToLogicalPointPtr)(HWND, LPPOINT);

	static PhysicalToLogicalPointPtr physical_to_logical_point_func = reinterpret_cast<PhysicalToLogicalPointPtr>(GetProcAddress(GetModuleHandleA("user32.dll"), "CloseTouchInputHandle"));

	if (physical_to_logical_point_func) {
		return (TRUE == physical_to_logical_point_func(hWnd, lpPoint));
	}
